...
export async function POST(request: NextRequest) {
  ...
  if (!(await rateLimit(requestKey(request, "newsletter"), 3, 10 * 60_000)).allowed) return NextResponse.json({ message: "Too many attempts. Please try later." }, { status: 429 });
  ...
  if (error?.code === "23505") return NextResponse.json({ message: "If this address can be subscribed, confirmation will follow." });
