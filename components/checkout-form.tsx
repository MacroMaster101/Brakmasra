...
export function CheckoutForm() {
  ...
  return <form className="checkout-form panel" onSubmit={submit}><h2>Secure checkout</h2><p>Payment details are handled entirely by our payment provider. BRAKMASRA never stores your card details.</p><button className="button button-primary full" disabled={!lines.length} type="submit">Continue to payment</button><p className="form-status" aria-live="polite">{status}</p></form>;
}
